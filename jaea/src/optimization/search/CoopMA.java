...
public class CoopMA extends Search {
	...
	public void search(Population pop) 
	{
        ...
        */
        ...
        {	    
        	...
        	// if (true || iGen >= DATA_BUILDING_PHASE) { //SMAnew2: suppose is SMA default, incremental popsize during DB building phase
        	if (iGen >= DATA_BUILDING_PHASE) { // SMAnew: suppose is SMA default, but no incremental popsize during DB building phase
        	...
        	else 
        	{
	        	...
	        	{
	        		// if crossover
	        		if (this.reproduce[rep] instanceof optimization.operator.population.Crossover)
	        		{
	/* CROSSOVER */
	...
	//	        	System.out.println("Crossover Estimation:");
		        	...
		        	for (int i = 0; i < nIndivs; i++)
		        		...
		        		{
		        			// fitness
		        			profileMetrices[i][rep][k][0] = metricG[k][0];
		        			// cost
		        			profileMetrices[i][rep][k][1] = metricG[k][1];
		        		}
	        		}
	        		else if (this.reproduce[rep] instanceof optimization.operator.population.Mutation)
	        		{
	/* MUTATION */
	...
	//	        		System.out.println("Mutation Estimation:");
		            	...
		            	for (int k =0; k < metricL.length; k++) {
		            		// fitness
		            		profileMetrices[i][rep][k][0] = metricL[k][0];
		            		// cost
		            		profileMetrices[i][rep][k][1] = metricL[k][1];
		            	}
		        	}
	        		}
	        	}
...
//	        				System.out.println("(" + u + "," + v + ") " + "FI: " + fi +	", Cost: " + profileMetrices[i][u][v][1]);
...
//	        				if (profileMetrices[i][u][v][1] >0) {
	...
	//        					double test = fi;
	        		...
	        		// data building phase - just random select profiles
	        		if (iGen < DATA_BUILDING_PHASE) randomMode = true;
...
//	        					+ iRep + ", iIL = " + iIL);
	        	...
	        	for (int rep = 0; rep < nRep; rep++)
	        	{
	        		if (this.reproduce[rep] instanceof optimization.operator.population.Crossover) {
			        	...
			        	for (int k = 0; k < nIL; k++) {
				        	if (list[rep][k].size() > 1) {
				        		// Crossover
				        		crossover.setPoolSize(poolsizeF * list[rep][k].size()); 
				        		list[rep][k] = crossover.doProcess(temp);	
				        		
					        	// Evaluation
					        	list[rep][k] = evaluation.doProcess(list[rep][k]);
					        	// Individual Learning
					        	int size = (int)Math.round(list[rep][k].size()*P_EXP);
		//			        	size = (size >= 1) ? size : 1;
					        	
					        	teacher[k].setPoolSize(size);
					        	list[rep][k] = teacher[k].doProcess(list[rep][k]);
				        	}
			        	}
	        		}
	        		else if (this.reproduce[rep] instanceof optimization.operator.population.Mutation) 
	        		{
			        	...
			        	for (int k = 0; k < nIL; k++) {
				        	if (list[rep][k].size() > 0) {
				        		// Crossover
				        		mutation.setFactor(poolsizeF);
					        	list[rep][k] = mutation.doProcess(list[rep][k]);
					        	// Evaluation
					        	list[rep][k] = evaluation.doProcess(list[rep][k]);
					        	// Individual Learning
					        	int size = (int)Math.round(list[rep][k].size()*P_EXP);
...
//		System.out.println();
	...
	public void initOperators()
	{
		...
		/* For Step function 
		this.reproduce[0] = this.mutation;
		 */
		/* Initialize Local Learning */
		int learnPoolsize = 5;
		int intensity = 300; //(default)
//		int intensity = 120;
//		int intensity = 150; // for Step function
		double accuracy = 1E-8;
		/* Set step size (default) */
		//double stepRange = (evalFunc.getUpBound() - evalFunc.getLowBound())/10; // large step
		double stepRange = (evalFunc.getUpBound() - evalFunc.getLowBound())/100;
		double stepsize = ((stepRange > 1.0) ? 1.0 : stepRange); 
		
		/* Set step size for Bump function 
		double stepsize = 0.7;
		*/
