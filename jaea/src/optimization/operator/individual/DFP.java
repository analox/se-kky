...
//		double [] x = {2.1, 4.9, 2.1};
		FitnessFunction f = 
			EARunTemplate.getFitnessFunc(FitnessFunction.F_Sphere, ndim, false);
		double [] s = new double[ndim];
		double shiftRange = (f.getUpBound() - f.getLowBound())/1000;
...
//						x[j] = Math.pow(-1, j)/Math.sqrt(x.length);
					double y = f.getFitnessFunc(x);
//					System.out.print(y);
					System.out.println("Initial: \n" + (new Matrix(x)).toString() + "->" + y);
		
					DFP directSearch = new DFP(f, 300);
