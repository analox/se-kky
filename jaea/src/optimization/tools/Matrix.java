...
public class Matrix {
	...
	public double getDeterminant() {
		...
		double [][] matrix = UpperTriangle(this.mat);
	...
	/**
	 * Gaussian transformation to triagle matrix - I guess
	 * @param input
	 * @return
	 */
	public double[][] UpperTriangle(double[][] input) {
		
		double f1 = 0;
		double temp = 0;
		int tms = input.length; // get This Matrix Size (could be smaller than
							// global)
		...
		// Copy
		for (int col = 0; col < tms ; col++) 
			for (int row = 0; row < tms; row++) 
				m[row][col] = input[row][col];
		// Calculate
	 ...
	 * @return
	 */
	static public double[][] Adjoint(double[][] a)  
	{
		int tms = a.length;
		...
		for (int i = 0; i < tms; i++)
			for (int j = 0; j < tms; j++) {
				...
				Matrix apMat = new Matrix(ap);
				det = apMat.getDeterminant();
				m[i][j] = Math.pow(-1, i + j) * det;
			}
		Matrix resMat = new Matrix(m);
		resMat = Matrix.transpose(resMat);
		return resMat.mat;
	}
	/**
	 ...
	 * @return
	 */
	static public Matrix inverse(Matrix aMat)  
	{
		...
		Matrix aWrap = new Matrix(a);
		double det = aWrap.getDeterminant();
		double dd = 0;
	...
	/**
	 * Read matrix from file in text format [row \n col \n matrix]
	 * With delimiter specified
	 */
	static public Matrix File2DMat(String filename, String delimiter) {
		try {
			FileReader fr = new FileReader(filename);
			BufferedReader input = new BufferedReader(fr);
			String s = "";
			...
			do {
				s = input.readLine();
				if (s != null) {
					String [] params = s.trim().split(delimiter);
					for (int k = 0; k < params.length; k++)
						matrix[count][k] = Double.parseDouble(params[k].trim());
					count++;	       
				}
			} 
			while (s != null);
...
//		double [][] Dmat = new double[][] {{1, 2, 3}, {7, 4, 2}, {6, 3, 9}};
//		Matrix D = new Matrix(Dmat);
//		Matrix Dinv = Matrix.inverse(D);
//		Matrix mulD = Matrix.multiply(D, Dinv);
//		System.out.println("D: \n" + D.toString());
//		System.out.println("Test: " + D.getDeterminant());
//		System.out.println("Dinv: \n" + Dinv.toString());
//		System.out.println("Multiply result: \n" + mulD.toString());
		
		Matrix res = Matrix.File2DMat("input_data/ackley_M_D50.txt", "\\s+");
