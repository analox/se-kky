...
public abstract class FitnessFunction {
	...
	public static final byte F_SchwefelNoise = 16;
	public static final byte F_Step = 17;
	public static final byte F_Bump = 18;
	...
	protected double [][]transMatrixT = null;
	/**
	 *  Shift vector 
	 */
	 ...
	 * @param mat
	 */
	public void setRotationMatrix(double [][] mat) 
	{
		this.transMatrix = mat;
		
		Matrix m = new Matrix(transMatrix);
		this.transMatrixT = (Matrix.transpose(m)).mat;
	}
	/**
...
//		System.out.println(res);
	...
	public void getFitnessGrad(double [] inputs, double resP, double [] g)
	{
		...
		/* OLD CODE - Calculate around point 
		...
		*/
		...
		for (i=0; i<ndim; i++) x_[i] = tInputs[i];
		...
		/* Get true gradients */
		if (transMatrixT != null)
			g = transformX(g, this.transMatrixT);
	}
	
	/**
	 * Assistant function - Compute computer precision
	 */
	private double fool(double x, double y) 
	{
		return (x*y + y);
	}
	...
	/**
	 * Transform vector of inputs x' = xM
	 * @param inputs (row vector)
		...
		else 
		{
			...
			for (int i = 0; i < inputs.length; i++)
			{
				...
				for (int j = 0; j < inputs.length; j++)
					if (Math.abs(inputs[j]) > 1E-9 && Math.abs(trans[j][i])> 1E-9)
						_inputs[i] += inputs[j]*trans[j][i];
	...
	{
		double res = 0;
		double [] _inputs = transformX(inputs, trans);
			...
			double [] shiftVector)
	{
		...
		double [] tInputs = transformX(sInputs, trans);
		res = getFitnessFunc(tInputs, offset);
		return res;
	}
